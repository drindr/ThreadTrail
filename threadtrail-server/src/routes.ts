...
// A stamp is deliberately approximate (nested untracked edits, >400 paths).
// Never let that approximation make a live result stale indefinitely.
const LIVE_CACHE_TTL_MS = 2000;

function cacheMatches(hit: CacheEntry | undefined, stamp: string): hit is CacheEntry {
  return !!hit && hit.stamp === stamp && (stamp === 'static' || Date.now() - hit.created < LIVE_CACHE_TTL_MS);
}

function cachePut(map: Map<string, CacheEntry>, key: string, entry: CacheEntry): void {
  if (Buffer.byteLength(entry.body) > MAX_CACHED_BODY_BYTES) return;
...
async function handle(req: IncomingMessage, res: ServerResponse, deps: { sessions: SessionStoreLike | undefined }): Promise<void> {
  ...
  if (parts[2] === 'records.json') {
    const key = JSON.stringify(['records', effectiveCwd, root]);
    // Coalesce the complete transaction, including stamping and cache writes.
    // Otherwise a request seeing a newer stamp can publish an older inflight
    // body under that stamp. Root belongs in the key because it's in the body.
    const body = await computeOnce(key, async () => {
      const stamp = await computeOnce(JSON.stringify(['stamp', effectiveCwd]), () => worktreeStamp(effectiveCwd));
      const hit = recordsCache.get(key);
      if (cacheMatches(hit, stamp)) return hit.body;
      const result = await collectRecords(effectiveCwd);
      result.root = root;
      const body = JSON.stringify(result);
      cachePut(recordsCache, key, { stamp, body, created: Date.now() });
      return body;
    });
  ...
  if (parts[2] === 'diff.json') {
    ...
    try {
      const key = JSON.stringify(['diff', effectiveCwd, from, to]);
      const body = await computeOnce(key, async () => {
        // Immutable commit diffs need neither stamps nor live-cache expiry.
        const stamp = from !== 'worktree' && to !== 'worktree' ? 'static'
          : await computeOnce(JSON.stringify(['stamp', effectiveCwd]), () => worktreeStamp(effectiveCwd));
        const hit = diffCache.get(key);
        if (cacheMatches(hit, stamp)) return hit.body;
        const body = JSON.stringify(await diffRecords(effectiveCwd, from, to));
        cachePut(diffCache, key, { stamp, body, created: Date.now() });
        return body;
      });
      sendRawJson(res, 200, body);
    } catch (err) {
